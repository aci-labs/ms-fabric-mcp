...
def get_sqlalchemy_connection_string(driver: str, server: str, database: str) -> Engine:
    ...
    engine = create_engine("mssql+pyodbc:///?odbc_connect={0}".format(params), connect_args={'attrs_before': attrs_before})
    return engine
...
async def get_sql_endpoint(
    ...
    try:
        ...
        resource_name = None
        endpoint = None
        workspace_name, workspace_id = await fabClient.resolve_workspace_name_and_id(workspace)
        if type and type.lower() == "lakehouse":
            client = LakehouseClient(fabClient)
            resource_name, resource_id = await fabClient.resolve_item_name_and_id(workspace=workspace_id, item=lakehouse, type="Lakehouse")
            lakehouse_obj = await client.get_lakehouse(workspace=workspace, lakehouse=resource_id)
            endpoint = lakehouse_obj.get("properties", {}).get("sqlEndpointProperties", {}).get("connectionString")
        elif type and type.lower() == "warehouse":
            client = WarehouseClient(fabClient)
            resource_name, resource_id = await fabClient.resolve_item_name_and_id(workspace=workspace_id, item=warehouse, type="Warehouse")
            warehouse_obj = await client.get_warehouse(workspace=workspace, warehouse=resource_id)
            endpoint = warehouse_obj.get("properties", {}).get("connectionString")
        if resource_name and endpoint:
            return resource_name, endpoint
        else:
            return None, f"No SQL endpoint found for {type} '{lakehouse or warehouse}' in workspace '{workspace}'."
